...
> 本文描述完整目标架构和施工合同。仓库中的`0.1.0`是可运行的参考核心，
> 已实现工作组账本、独立验证、只读前端、本地长期记忆、因果索引与Graphiti
> 请求生成；LoopX真实对账、原始session归档分类和宿主项目权威适配仍是待接
> 入扩展。本文中的目标字段与示例不应被误读为所有可选组件已经随包交付。
...
| 宿主项目权威层 | 用户决定、正式状态、稳定handoff、发布和接纳 | 临时协作过程 |
...
└─ host_control_plane/                  # 若项目已有正式控制面则沿用
```
...
3. **正式真值不被污染**：协调完成、工作组共识和长期记忆都必须经过独立审核，才能影响宿主项目权威层。
