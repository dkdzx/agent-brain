...
class ConstructionBundleStoreTests(unittest.TestCase):
    ...
    def test_html_contains_three_views_and_read_only_notice(self):
        ...
        self.assertIn("function applyGraphScale()", html)
        self.assertIn("function setGraphScale(next,anchorClientX=null,anchorClientY=null)", html)
        self.assertIn("setGraphScale(S.scale*factor,event.clientX,event.clientY)", html)
        wheel_start = html.index("vp.addEventListener('wheel'")
        wheel_end = html.index("},{passive:false});", wheel_start)
        wheel_handler = html[wheel_start:wheel_end]
        self.assertNotIn("renderGraph()", wheel_handler)
        self.assertIn("event.stopPropagation()", wheel_handler)
