...
DEFAULT_RUNTIME_ROOT = Path(os.environ.get("AGENT_BRAIN_RUNTIME_ROOT", str(Path.cwd() / "runtime"))).resolve()
...
PUBLIC_GROUP_NAMES = {
    "demo-workgroup-001": "工作组一",
}
PUBLIC_GROUP_INSTANCE_LABELS = {
    "demo-workgroup-001": "真实协作演示",
}
...
def group_display_title(group: dict[str, Any], group_id: str) -> str:
    if group_id in PUBLIC_GROUP_NAMES:
        return PUBLIC_GROUP_NAMES[group_id]
...
def group_instance_label(group_id: str) -> str:
    if group_id in PUBLIC_GROUP_INSTANCE_LABELS:
        return PUBLIC_GROUP_INSTANCE_LABELS[group_id]
    if group_id.startswith("canary-"):
        suffix = group_id.rsplit("-", 1)[-1]
        return f"试运行组{suffix}"
    ...
    return {
        ...
        "agent_management_projection": {
            ...
            "source_summary": {
                ...
                "todo_source": "Agent Brain active workgroup runtime",
            },
            "workgroup_summary": {
                "display_name": "工作组",
